<?php
session_start();
if(isset($_POST['pass_btn']) && isset($_SESSION['userId'])) {
    ...
    if($mobile_flag) {
        header('Location: ../pass-form.php?error=moblen');
        exit();         
    }
    ...
    for($i=0;$i<$date_len;$i++) {        
        ...
        if($flag) {
            header('Location: ../pass-form.php?error=invdate');
            exit();    
            break;
        }      
    }        
    ...
    if(is_null($pass_id)) {
        ...
        $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt,$sql)) {
            header('Location: ../pass-form.php?error=sqlerror');
            exit();            
        } else {         
            mysqli_stmt_execute($stmt);
        }        
